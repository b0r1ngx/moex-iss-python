    ...
    def auth(self):
        ...
        if self.config.proxy_url:
            opener = urllib2.build_opener(
                urllib2.ProxyHandler({"http": self.config.proxy_url}),
        ...
        opener.addheaders = [(
            ...
            'Basic %s' % base64.b64encode(
                (self.config.user + ':' + self.config.password).encode('utf-8')
            ).decode('utf-8')
        )]
...
class MicexISSDataHandler:
    ...
    def do(self):
        """ This handler method should be overridden to perform the processing of data returned by the server."""
        pass
...
class MicexISSClient:
    ...
    def __init__(self, config, auth, handler, container):
        ...
        if config.proxy_url:
            self.opener = urllib2.build_opener(
                urllib2.ProxyHandler({"http": config.proxy_url}),
    ...
    def get_history_securities(self, engine, market, board, date):
        ...
        url = requests['history_secs'] % {'engine': engine, 'market': market, 'board': board, 'date': date}
        ...
        while cnt > 0:
            ...
            result = []
            for sec in jdata:
                result.append((sec[secIdx], del_null(sec[closeIdx]), del_null(sec[tradesIdx])))
